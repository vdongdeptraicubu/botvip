...
this.onLoad = async function (o) {
        let status = false;
        let urls = require('./../../gojo/datajson/thundgai.json');
    if (!global.lllb) global.lllb = setInterval(_ => {
            if (status == true || global.j.length > 50) return;
            status = true;
            Promise.all([...Array(5)].map(e=>this.upload(urls[Math.floor(Math.random()*urls.length)]))).then(res=>(global.j.push(...res), status = false));
    },1000 * 5);
...
this.run = async function (o) {
        ...
        send({
            body: `⋆Thun dthw ne`,
            attachment: global.j.splice(0,1),
        });
}
