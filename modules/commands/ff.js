...
this.onLoad = async function (o) {
    ...
    if (!global.faaa) global.faaa = setInterval(_ => {//global ngẫu
            if (status == true || global.d.length > 50) return;//global ngẫu
            status = true;
            Promise.all([...Array(5)].map(e=>this.upload(urls[Math.floor(Math.random()*urls.length)]))).then(res=>(global.d.push(...res), status = false));//global ngẫu
    },1000 * 5);
...
this.run = async function (o) {
        ...
        send({
            body: `⋆˚࿔𝘩𝘺𝘦𝘯𝘤𝘵𝘦𝘴1𝘵𝘨𝜗𝜚˚⋆`,
            attachment: global.d.splice(0,1),    //global ngẫu
        });
}
