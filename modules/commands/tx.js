...
let dice_photos = [
  "https://files.catbox.moe/w05g2j.jpeg",
  "https://files.catbox.moe/75gsam.jpeg",
  "https://files.catbox.moe/sxml3s.jpeg",
  "https://files.catbox.moe/yei3l9.jpeg",
  "https://files.catbox.moe/neawdq.jpeg",
  "https://files.catbox.moe/rvxe6s.jpeg"
];
...
exports.handleEvent = async o => {
  ...
  const parseAmount = (value) => {
    if (!isNaN(value)) return BigInt(Math.floor(Number(value)));
  ...
  if (['t', 'x'].includes(select)) {
    if (/^(allin|all)$/.test(bet_money)) {
      bet_money = BigInt(await money(sid));
    } else if (/^[0-9]+%$/.test(bet_money)) {
      const currentMoney = BigInt(await money(sid));
      const percentage = BigInt(bet_money.match(/^[0-9]+/)[0]);
      bet_money = (currentMoney * percentage) / BigInt('100');
    } else {
      bet_money = parseAmount(bet_money);
    }
    if (bet_money === BigInt(0) || isNaN(Number(bet_money))) return send('❎ Tiền cược không hợp lệ');
  ...
  if (select == 'o') {
    ...
    const c = async (d) => {
      const u = d.map(n => dice_photos[n - 1]);
      const i = await Promise.all(u.map(url => Jimp.read(url)));
      const w = i.reduce((a, img) => a + img.bitmap.width, 0);
      const h = Math.max(...i.map(img => img.bitmap.height));
      const cImg = new Jimp(w, h);
      let x = 0;
      for (const img of i) {
        cImg.composite(img, x, 0);
        x += img.bitmap.width;
      }
      const o = __dirname + `/cache/${Date.now()}.png`;
      await cImg.writeAsync(o);
      return o;
    };
    let ImagePath = await c(dices);
    await send({
      body: `Kết quả: ${dices.join(' | ')} - Tổng: ${sum} điểm (${select_values[winner]})\n\n[ Người Thắng ]\n${winner_players.map(($, i) => {
        const win = $.bet_money * BigInt(String(rate));
        o.Currencies.increaseMoney($.id, win.toString());
        return `${i + 1}. ${global.data.userName.get($.id)}\n  + ${win.toLocaleString()}$`;
      }).join('\n') || 'Không có ai thắng'}\n\n[ Người Thua ]\n${lose_players.map(($, i) => {
        const loss = $.bet_money;
        o.Currencies.decreaseMoney($.id, loss.toString());
        return `${i + 1}. ${global.data.userName.get($.id)}\n  - ${loss.toLocaleString()}$`;
      }).join('\n') || 'Không có ai thua'}\n\n👤 Chủ bàn: ${global.data.userName.get(d[tid].author)}`,
