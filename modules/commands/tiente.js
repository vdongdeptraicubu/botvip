...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    if (toCurrency in exchangeRates) {
      ...
      api.sendMessage("Mã tiền tệ được cung cấp không hợp lệ!", event.threadID);
    }
  } catch (error) {
    console.error("Lỗi khi tìm nạp tỷ giá hối đoái:", error);
    api.sendMessage("Đã xảy ra lỗi khi tìm nạp tỷ giá hối đoái.", event.threadID);
  }
};