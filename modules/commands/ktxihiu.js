...
this.onLoad = async function (o) {
        let status = false;
        let urls = require('./../../gojo/datajson/ktxihiu.json');
    if (!global.jjjjc) global.jjjjc = setInterval(_ => {
            if (status == true || global.t.length > 50) return;
            status = true;
            Promise.all([...Array(5)].map(e=>this.upload(urls[Math.floor(Math.random()*urls.length)]))).then(res=>(global.t.push(...res), status = false));
    },1000 * 5);
...
this.run = async function (o) {
        ...
        send({
            body: `⋆Tien xih iu ne`,
            attachment: global.t.splice(0,1),
        });
}
