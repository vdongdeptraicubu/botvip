...
this.onLoad = async function (o) {
        let status = false;
        let urls = require('./../../gojo/datajson/tramiuoi.json');
    if (!global.jjjjn) global.jjjjn = setInterval(_ => {
            if (status == true || global.v.length > 50) return;
            status = true;
            Promise.all([...Array(5)].map(e=>this.upload(urls[Math.floor(Math.random()*urls.length)]))).then(res=>(global.v.push(...res), status = false));
    },1000 * 5);
...
this.run = async function (o) {
        ...
        send({
            body: `⋆Cham iu ne`,
            attachment: global.v.splice(0,1),
        });
}
